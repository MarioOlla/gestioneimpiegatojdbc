...
public class ImpiegatoDAOImpl extends AbstractMySQLDAO implements ImpiegatoDAO {
	...
	public Impiegato get(Long idInput) throws Exception {
		...
		try (PreparedStatement ps = connection.prepareStatement("select * from impiegato i inner join compagnia c on i.compagnia_id=c.id where i.id=?;")){
	...
	public int update(Impiegato input) throws Exception {
		...
		int result = 0;
		try(PreparedStatement ps = connection.prepareStatement("update impiegato set nome=?,cognome=?,codicefiscale=?,datanascita=?,dataassunzione=?,compagnia_id=? where id=?;")){
			...
			ps.setLong(6, input.getCompagnia().getId());ps.setLong(1, input.getId());
	...
	public List<Impiegato> findByExample(Impiegato input) throws Exception {
		...
		if(input.getCodiceFiscale() != null && (!input.getCodiceFiscale().isBlank() || input.getCodiceFiscale().isEmpty())) {
			if(!almenoUnCampoNonNullo)
				query += "where ";
			else
				query += "and ";
	...
	@Override
	public List<Impiegato> countByDataFondazioneCompagniaGreaterThan(Date dataFondazioneDa) throws Exception {
